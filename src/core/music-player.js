(function (global) {
  ...
  Sequence.prototype.createFxNodes = function () {
    var eq = [
      ...
      prev = (this.gain = window.ac.createGain());
    this.wet = window.ac.createGain();
    this.dry = window.ac.createGain();
    eq.forEach(
  ...
  Sequence.prototype.createOscillator = function () {
    this.stop();
    this.osc = window.ac.createOscillator();
...
export const Music = {
  init: function () {
    ...
    if (window.ac) {
      ...
      this.ac = ac;
      this.osc = window.ac.createOscillator();
      this.compressor = window.ac.createDynamicsCompressor();
      this.output = window.ac.createGain();
      this.output.connect(window.ac.destination);
      ...
      // this.lead.gain.gain.value = 1.0 / 2;
      // this.lead.createCustomWave([-0.8, 1, 0.8, 0.8, -0.8, -0.8, -1]);

      this.bass.staccato = 0.1;
      this.bass.smoothing = 0.2;
      // this.bass.gain.gain.value = 0.75 / 2;
  ...
  lead: new Sequence(window.ac, tempo, [
    "F#4 q",
    "C#5 q",
    "F#4 q",
    "E5 q",
  ...
  harmony: new Sequence(window.ac, tempo, [
    "F#3 w",
    "F#3 w",
    "B2 w",
    "C#3 w",
  ...
  bass: new Sequence(window.ac, tempo, [
    "F#2 h",
    "F#2 h",
    "F#2 h",
    "F#2 h",

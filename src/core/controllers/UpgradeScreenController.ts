...
export class UpgradeScreenController {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    // Title
    drawEngine.drawText("Found loot", 12, drawEngine.getCenterX(), 20);
    ...
    this.upgrades.forEach((upgrade, index) => {
      ...
      ctx.fillRect(ctx.canvas.width / 2 - 200, y, 400, boxHeight);

      ctx.strokeStyle = "white";
      ctx.lineWidth = 2;
      ctx.strokeRect(ctx.canvas.width / 2 - 200, y, 400, boxHeight);

      ctx.fillStyle = "white";
      ctx.font = "20px Arial";
      ctx.textAlign = "left";
      ctx.fillText(upgrade.name, ctx.canvas.width / 2 - 180, y + 30);

      ctx.font = "16px Arial";
      ctx.fillText(upgrade.description, ctx.canvas.width / 2 - 180, y + 60);
    });
